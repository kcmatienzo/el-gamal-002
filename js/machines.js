...
var wire = {
  keyDisplay: findElementsByClass(
    document.getElementById("keyDisplay"),
    "data"
  )[0],
  encryptedMsgDisplay: findElementsByClass(
    document.getElementById("encryptedMsg"),
    "data"
  )[0],
  MESSAGES: {
    MESSAGE: "message",
...
var wire2 = {
  keyDisplay: findElementsByClass(
    document.getElementById("keyDisplay2"),
    "data"
  )[0],
  encryptedMsgDisplay: findElementsByClass(
    document.getElementById("encryptedMsg2"),
    "data"
  )[0],
  MESSAGES: {
    MESSAGE: "message",
...
wire.render = function () {
  wire.keyDisplay.innerHTML =
    ...
    " y: " +
    this.publicKey.h;
  wire.encryptedMsgDisplay.innerHTML =
    "u: " + this.message.b + ", v: " + this.message.c;
};

wire2.render = function () {
  wire2.keyDisplay.innerHTML =
    "p: " +
    this.publicKey.p +
    " g: " +
    this.publicKey.g +
    " y: " +
    this.publicKey.h;
  wire2.encryptedMsgDisplay.innerHTML =
    "m: " + this.message.m + "r: " + this.message.r + ", s: " + this.message.s;
};
...
wire.subscribe = function (message, callback) {
  wire.subscribers[message] = wire.subscribers[message]
    ? wire.subscribers[message].concat([callback])
    : [callback];
};

wire2.subscribe = function (message, callback) {
  wire2.subscribers[message] = wire2.subscribers[message]
    ? wire2.subscribers[message].concat([callback])
    : [callback];
};
...
(function () {
  ...
  bind(
    receiver.getGeneratorBtn,
    "click",
    function (e) {
      p = parseInt(
        document
          .getElementById(receiver.primeFieldId)
          .getElementsByTagName("input")[0].value,
        10
      );
      document.getElementById(receiver.primeFieldId).className = document
        .getElementById(receiver.primeFieldId)
      ...
      try {
        ...
        ElGamal.getAllRootsAsync(p, function (roots) {
          generatorListEl.innerHTML += roots
            .map(function (r) {
  ...
  bind(
    receiver.getPublicKeyBtn,
    "click",
    function (e) {
      var errorFields = findElementsByClass(receiver.el, "error"),
        i = 0,
        publicKey;
      ...
      x = parseInt(
        document
          .getElementById(receiver.privateKeyFieldId)
          .getElementsByTagName("input")[0].value,
        10
      );
  ...
  bind(
    ...
    function (e) {
      if (!wire.message) {
        alert("You haven't sent a message yet.");
        return false;
      }
      findElementsByClass(
        document.getElementById("messageDisplay"),
        "data"
      )[0].innerHTML = "Decrypted Message: " + eg.decrypt(wire.message);
  ...
  bind(receiver.randomPrivateKeyBtn, "click", function (e) {
    ...
    document
      .getElementById(receiver.privateKeyFieldId)
      .getElementsByTagName("input")[0].value = key;
  });
})();
...
(function () {
  ...
  bind(
    sender.encryptMsgBtn,
    "click",
    function (e) {
      if (!wire.publicKey) {
        alert("You need the public key to send a message.");
        return false;
      }

      var errorFields = findElementsByClass(sender.el, "error"),
        i = 0,
        message = parseInt(
          document
            .getElementById(sender.messageFieldId)
            .getElementsByTagName("input")[0].value,
          10
        ),
        privateKey = parseInt(
          document
            .getElementById(sender.privateKeyFieldId)
            .getElementsByTagName("input")[0].value,
          10
        );
  ...
  bind(
    ...
    function (e) {
      ...
      findElementsByClass(
        ...
        "Checking Signatures: " +
        ElGamal.is_valid(wire2.message, wire2.publicKey);
...
(function () {
  ...
  bind(
    sender2.getGeneratorBtn,
    "click",
    function (e) {
      p = parseInt(
        document
          .getElementById(sender2.primeFieldId)
          .getElementsByTagName("input")[0].value,
        10
      );
      document.getElementById(sender2.primeFieldId).className = document
        .getElementById(sender2.primeFieldId)
        .className.replace(/error/, "");
      ...
      try {
        sender2.generatorList.innerHTML =
          '<select id="generatorSelect2"></select>';
  ...
  bind(
    sender2.getPublicKeyBtn,
    "click",
    function (e) {
      var errorFields = findElementsByClass(sender2.el, "error"),
        i = 0,
        publicKey;
      ...
      x = parseInt(
        document
          .getElementById(sender2.privateKeyFieldId)
          .getElementsByTagName("input")[0].value,
        10
      );
  ...
  bind(
    sender2.encryptMsgBtn,
    "click",
    function (e) {
      if (!wire2.publicKey) {
        alert(
          "Для начала вам нужно получить публичный ключ, чтоб отправить сообщение"
        );
        return false;
      }
      ...
      var errorFields = findElementsByClass(sender2.el, "error"),
        ...
        message = parseInt(
          document
            .getElementById(sender2.messageFieldId)
            .getElementsByTagName("input")[0].value,
          10
        ),
        privateKey = parseInt(
      ...
      tempKey = parseInt(
        document
          .getElementById(sender2.tempKeyFieldId)
          .getElementsByTagName("input")[0].value,
        10
      );
